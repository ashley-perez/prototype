...
function update() {
  if (!ticks) {
        ...
        if (enemies.length === 0) {
          for (let i = 0; i < 5; i++) {
              ...
              enemies.push({
                  pos: vec(posX, posY), speed: gameConstants.NORMAL_BULLET_SPEED, coolDown: rnd(55, gameConstants.FIRE_RATE)
              });
          }
        }
  ...
  if (start) {
    ...
    remove(enemies, (e) => {
        ...
        if (e.pos.x > gameConstants.WIDTH || e.pos.x < 0 || e.pos.y > gameConstants.HEIGHT || e.pos.y < 0) {
    ...
    remove(coins, (c) => {
     ...
     remove(eBullets, (eb) => {
        ...
        const isCollidingWithPlayer
            = char("b", eb.pos, {rotation: eb.rotation}).isColliding.rect.green;
        if (isCollidingWithPlayer) {
            end();
            play("powerUp");
        }
